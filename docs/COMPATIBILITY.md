...
| Session startup, logout, and recovery | Behaviorally equivalent | The namespaced `wtwm-session` entrypoint supervises exactly one foreground compositor and returns its status to the login manager without a restart loop. Managed logins publish the live Wayland/Xwayland displays and namespaced XDG desktop identity to D-Bus/systemd activation before `-s`, while the packaged GTK portal policy gives portal-backed applications a backend that can open this session. Direct nested launches do not overwrite persistent activation state. Complete configuration validation and runtime initialization precede `-s`; a failed startup child does not end the compositor. `f.quit` and INT/HUP/QUIT/TERM perform orderly success cleanup, while startup failures return nonzero without running `-s` or replacing saved state |
